...
const freetype = if (font.options.backend.hasFreetype()) @import("freetype") else void;
    ...
    pub const KeyEvent = struct {
        ...
        fn physicalKey(self: KeyEvent) input.Key {
            if (self.keycode & keycode_physical_key_flag != 0) {
                ...
                return std.enums.fromInt(input.Key, key_raw) orelse .unidentified;
            }
...
pub const Platform = union(PlatformTag) {
    ...
    pub const OpenGL = if (builtin.target.os.tag == .linux) void else struct {
        userdata: ?*anyopaque,
    ...
    pub const C = if (builtin.target.os.tag == .linux) extern union {
        macos: extern struct {
            nsview: ?*anyopaque,
        },
        ios: extern struct {
    ...
    pub fn init(tag_int: c_int, c_platform: C) !Platform {
        ...
        if (builtin.target.os.tag == .linux and tag_int > 3)
...
pub const PlatformTag = enum(c_int) {
    ...
    macos = 1,
    ios = 2,
    opengl = if (builtin.target.os.tag == .linux) 6 else 3,
    metal_external = 4,
...
comptime {
    ...
    if (@sizeOf(Platform.C) != if (builtin.target.os.tag == .linux) 32 else 40)
...
pub const Surface = struct {
    ...
    clipboard_request_mutex: std.Io.Mutex = .init,
    clipboard_requests: std.ArrayListUnmanaged(ClipboardRequestEntry) = .empty,
    ...
    pub const Options = extern struct {
        ...
        pty_tee_cb: if (builtin.target.os.tag == .linux)
            void
        else
            ?PtyTeeCallback = if (builtin.target.os.tag == .linux)
        {} else null,
        pty_tee_userdata: if (builtin.target.os.tag == .linux)
            void
        else
            ?*anyopaque = if (builtin.target.os.tag == .linux)
        {} else null,
    ...
    pub fn init(
        ...
        const io_mode = if (comptime builtin.target.os.tag == .linux)
            std.enums.fromInt(IoMode, opts.io_mode) orelse return error.InvalidIoMode
        else
            opts.io_mode;
        ...
        self.* = .{
            ...
            .pty_tee_cb = if (comptime builtin.target.os.tag == .linux) null else opts.pty_tee_cb,
            .pty_tee_userdata = if (comptime builtin.target.os.tag == .linux) null else opts.pty_tee_userdata,
            .initial_output = initial_output,
            .destroying = .init(false),
            .clipboard_request_mutex = .init,
            .clipboard_requests = .empty,
    ...
    test "embedded surface scrollback cap inherits when unset" {
        ...
        try std.testing.expectEqual(
            @as(usize, if (builtin.target.os.tag == .linux) 160 else 168),
            @sizeOf(Options),
        );
        try std.testing.expectEqual(
            @as(usize, 50_000_000),
    ...
    fn registerClipboardRequest(
        ...
        self.clipboard_request_mutex.lockUncancelable(global.io());
        defer self.clipboard_request_mutex.unlock(global.io());
    ...
    fn drainClipboardRequests(self: *Surface) void {
        ...
        self.clipboard_requests = .empty;
        self.clipboard_request_mutex.unlock(global.io());
    ...
    pub fn updateGridSize(self: *Surface, columns: u16, rows: u16) bool {
        ...
        const screen = self.core_surface.size.screenForGrid(requested) orelse
            return false;
        self.updateSize(screen.width, screen.height) catch return false;
        ...
        if (!self.core_surface.size.grid().equals(requested)) {
            const adjusted = self.core_surface.size.screenForGrid(requested) orelse
                return false;
            self.updateSize(adjusted.width, adjusted.height) catch return false;
        }
...
const surface_config_abi_size = if (builtin.target.os.tag == .linux) 160 else 168;
...
test "embedded surface config ABI is pinned" {
    ...
    if (builtin.target.os.tag == .linux) {
        try std.testing.expectEqual(@as(c_int, @intFromEnum(IoMode.exec)), defaults.io_mode);
    } else {
...
comptime {
    ...
    if (builtin.target.os.tag != .linux and defaults.io_mode != .exec)
...
pub const CAPI = struct {
    ...
    fn cEnum(
        ...
        return std.enums.fromInt(T, raw) orelse {
    ...
    export fn ghostty_surface_size(surface_: ?*Surface) SurfaceSize {
        const surface = surfaceHandle(surface_, "ghostty_surface_size") orelse
            return std.mem.zeroes(SurfaceSize);
        ...
        while (try it.next(alloc)) |run| {
            ...
            if (run.font_index.special()) |special| {
                ...
                const ps_name: []const u8 = blk: {
                    if (comptime font.options.backend.hasFreetype()) {
                        const ptr = freetype.c.FT_Get_Postscript_Name(face.face.handle);
                        break :blk if (ptr == null) "" else std.mem.span(ptr);
                    }
                ...
                const url_path: ?[]const u8 = blk: {
                    if (comptime !font.options.backend.hasFreetype() and font.options.backend.hasCoretext()) {
    ...
    export fn ghostty_surface_key_consume_if_menu_action(
        ...
        const key_event = event.keyEvent("ghostty_surface_key_consume_if_menu_action") orelse return false;
        const core_event = key_event.core() orelse {
            log.warn("error processing key event", .{});
            return false;
        };
    ...
    const Linux = struct {
        ...
        export fn ghostty_inspector_opengl_render(ptr_: ?*Inspector) bool {
            ...
            defer ptr.doneOpenGLContext();
            ptr.renderOpenGL() catch {
                log.warn("OpenGL inspector render requested before backend initialization", .{});
                return false;
            };
            return true;
        }
...
test "CAPI clipboard completion consumes request on rejected text" {
    ...
    surface.app = &app;
    surface.destroying = .init(false);
    surface.clipboard_request_mutex = .init;
    surface.clipboard_requests = .empty;
