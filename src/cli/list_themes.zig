...
fn postCmuxReloadNotification(
    ...
    const name = NSString.msgSend(
        ...
        .{@as([*:0]const u8, "com.cmuxterm.themes.reload-config")},
    );
...
const Preview = struct {
    ...
    fn syncCmuxPreview(self: *Preview) !void {
        const cmux = &(self.cmux orelse return);
                ...
                switch (self.mode) {
                    .normal => {
                        ...
                        if (self.cmux != null and key.matches('t', .{})) {
                            self.cmux.?.target_mode = switch (self.cmux.?.target_mode) {
                                .both => .light,
                                .light => .dark,
                                .dark => .both,
                            };
